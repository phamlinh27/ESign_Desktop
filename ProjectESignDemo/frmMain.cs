    ...
    public partial class frmMain : Form
    {
        ...
        private void btnPathChooseSign_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ofd.Filter = "Document file (*.pdf;*.xml;*.docx;*.xlsx)|*.pdf;*.xml;*.docx;*.xlsx";
        ...
        private CalculateLocalHashResult hash_document_api(CertificateRespone cert)
        {
            ...
            var calculateLocalHashData = new CalculateLocalHashData_Api
            {
                Certificate = cert.certificate,//Convert.FromBase64String(cert.certificate),
                CertificateChain = cert.certiticateChain,
                PdfDocs = list_pdf.Count > 0 ? list_pdf: null,
                XmlDocs = list_xml.Count > 0 ? list_xml: null,
                ExcelDocs = list_excel.Count > 0 ? list_excel : null,
                WordDocs = list_word.Count > 0 ? list_word : null,
            };
            ...
            // Gọi api hash file
            CalculateLocalHashResult info_file_hase = Program.RESTful_Action
                    .Call_Request<CalculateLocalHashData_Api, CalculateLocalHashResult>(HttpMethod.Post,
                    Program.Configuration["URL_HASHING"],
                    header_Params,
                    calculateLocalHashData,DataType.JSON).data;
        ...
        private void attach_digital_signature_api(CertificateRespone cert, CalculateLocalHashResult info_file_hash, SignChecker signchecker)
        {
            var attachPdfDatas = new List<AttachSignaturePdfData_Api>();
            var attachXmlDatas = new List<AttachSignatureXmlData_Api>();
            var attachWordDatas = new List<AttachSignatureWordData_Api>();
            var attachExcelDatas = new List<AttachSignatureExcelData_Api>();
            ...
            switch (TypeFile)
            {
                ...
                case DocumentType.Xml:
                    ...
                    attachXmlDatas.Add(new AttachSignatureXmlData_Api()
                    {
                ...
                case DocumentType.Word:
                    ...
                    attachWordDatas.Add(new AttachSignatureWordData_Api()
                    {
                ...
                case DocumentType.Excel:
                    ...
                    attachExcelDatas.Add(new AttachSignatureExcelData_Api()
                    {
            ...
            var attachSignatureData =new AttachSignatureData_Api()
            {
                PdfDocs = attachPdfDatas.Count > 0 ? attachPdfDatas: null,
                XmlDocs = attachXmlDatas.Count > 0 ? attachXmlDatas : null,
                WordDocs = attachWordDatas.Count > 0 ? attachWordDatas : null,
                ExcelDocs = attachExcelDatas.Count > 0 ? attachExcelDatas : null,
            ...
            // Gọi api ký
            AttachSignatureResult attachSignatureResult = Program.RESTful_Action
                    .Call_Request<AttachSignatureData_Api, AttachSignatureResult>(HttpMethod.Post,
                    Program.Configuration["URL_ATTACH_SIGNATURE"],
                    header_Params,
                    attachSignatureData,
                    DataType.JSON).data;
